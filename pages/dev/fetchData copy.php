...
if(isset($_POST['searchmedico'])){
    ...
    while($row = mysqli_fetch_array($result) ){
        $response[] = array("label"=>$row['nome'],"value"=>$row['idmedico'],"cpfMedico"=>$row['cpf'],"crm"=>$row['crm'], "ufCrm"=>$row['ufCrm']);
    }
...
if(isset($_POST['searchhospital'])){
    $search = $_POST['searchhospital'];

    $query = "SELECT * FROM hospital WHERE hospital like'%".$search."%'";
    $result = mysqli_query($conn,$query);
    
    while($row = mysqli_fetch_array($result) ){
        $response[] = array("label"=>$row['hospital'], "value"=>$row['cnpj']);
    }
