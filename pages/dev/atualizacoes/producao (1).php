			...
			<div class="modal fade" id="modalXml" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
							<div class="modal-dialog" role="document">
								...
								<div class="form-group col-md-6">
									...
									<div class="form-group col-md-6">
												<label for="nome">Nome Documento</label>
												<input class="form-control" type="text" name="pdfff" id="pdfff">
											</div>
...
$(document).ready(function(){
	...
	$('#adicionarProcedimentos').on( 'click', function () {
		...
		var td = tblProcedimentos.find('tbody tr').find('td');
	...
	var valor = parseFloat(td.eq(5).data('value')); // pega o valor 
		...
		console.log('CADE TU: ',cell.data(conta.toFixed(2))); 
			...
			$('#totalProcedimentos').html(parseFloat(soma).toFixed(2))
			console.log('VALOR TOTAL PROCEDIMENTO:', soma);
	});
...
function calcularPorcentagem(tr){
	...
	//console.log('ValorTotal:', conta );

}
	// REDUZIR PORCENTAGEM

	/*$("table#tblprocedimentos").on("change", "td.reduzirPorcentagem input, td.adicionarPorcentagem input" , function(){
	
	 var red = $(this).val(); //Valor input 
	 ...
	 var row = cell.index().row;
	 var tr = $("table#tblprocedimentos").find('tbody tr').eq(row);

		return calcularPorcentagem(tr);

		//console.log('Célula Seguinte:', cell);
	 var valor = parseFloat($(this.parentElement.nextSibling).data("value")); // Pega o valor da Celula
	 console.log('Valor: ', valor);
	 
	 var ad = Table.cell(this.parentElement.previousSibling).data(); // Determina Célula Anterior
	console.log('AD: ',ad);
...
/*
	$("table#tblprocedimentos").on("change", "td.adicionarPorcentagem input", function(){
	 ...
	 var cell = Table.cell(this.parentElement.nextSibling.nextSibling); // Determina Célula seguinte
	
	console.log('adiconarPorcentagem:' , cell);
	 var valor = parseFloat($(this.parentElement.nextSibling.nextSibling).data("value")); // Pega o valor da Celula
	 console.log('Valor: ', valor);
...
// adiciona varios procedimentos
	...
	//console.log('Célula Seguinteeee:', cell);
	// var valor2 = parseFloat($(this.parentElement.nextSibling).data("value")); // Pega o valor da Celula
...
<script>
		$(document).on('click','#btnEnviarXml',function(e){
			e.preventDefault();
				   ...
				   var pdfff =$("#pdfff").val();
					window.open("envioXmlProducao.php?id="+idconvenio+"&start_date="+start_date+"&end_date="+end_date+"&pdfff="+pdfff);
		});
</script>
