...
$nome = "ELETIVAS-CSV-20191122.csv";
...
/*  
  $sql2 = "INSERT IGNORE INTO producao (idempresa, dataRealizacao,idpaciente, idmedico, paciente, 
          ...
          '$dados[6]', (SELECT idconvenio FROM convenio WHERE descricao='PARTICULAR CAM' LIMIT 1),  'PARTICULAR CAM', '$dados[12]', '$dados[7]', REPLACE(REPLACE('$dados[8]', 'R$', ''), ',', '.'),
          REPLACE(REPLACE('$dados[8]', 'R$', ''), ',', '.'), STR_TO_DATE('$dados[2]', '%d/%m/%Y %H:%i:%s'), STR_TO_DATE('$dados[2]', '%d/%m/%Y %H:%i:%s'),  null,  '$dados[3]', '$dados[9]')";
...
*/
 ...
 /* $sql2 = "INSERT IGNORE INTO producao (idempresa, dataRealizacao,idpaciente, idmedico, paciente, 
  medico,idconvenio, convenio, hospital, descricaoProcedimento, valorProcedimento, 
  valorRecebido, dataPagamento, dataRepasse, statusPagamento, observacao)
  ...
  '$dados[10]', (SELECT idconvenio FROM convenio WHERE descricao='$dados[12]' LIMIT 1),  '$dados[12]', '$dados[12]', '$dados[14]', REPLACE(REPLACE('$dados[15]', 'R$', ''), ',', '.'),
  (IF(STRCMP ('$dados[17]', 'PAGO')=0, REPLACE(REPLACE('$dados[15]', 'R$', ''), ',', '.'),0.00)), STR_TO_DATE('$dados[16]', '%d/%m/%Y %H:%i:%s'),
