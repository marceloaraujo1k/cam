   ...
   <div id="wrapper">
			...
			<div class="panel-body">
				<div class="well">	
						...
						<div class="row"> 
								<div class="form-group col-lg-2">
									...
									<div class="form-group col-lg-1">
										<button id="add" class="btn btn btn-sm btn-primary" data-toggle="modal" data-target="#modalAdicionaProcedimentos"><i class="glyphicon glyphicon-plus"></i></button>
						...
						<div class="form-group col-lg-2"> 
								...
								<!-- <input type="submit" name="submit" value="inserirConta"  class="btn btn-success"> -->
								</div>
							</div>
						</form>	
	...
	<!-- Bootstrap Modal - To Add New Record -->
			<!-- Modal Relatorios -->
				...
				<div class="modal fade" id="modalAdicionaProcedimentos" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
							<div class="modal-dialog modal-lg" role="document">
								<div class="modal-content">
									<div class="modal-header">
										<h4 class="modal-title" id="modalTitle">Adicionar Procedimentos</h4>
									</div>
									<div class="modal-body">
									<form role="form" id="formAdicionaProcedimentos" action="./.php" method='post' enctype="multipart/form-data">
									
								<div class="row">
									<div class="form-group col-md-5">
										<label for="nome">Cód.</label>
										<input class="form-control"  id="searchCodProcedimento1" placeholder="Código Procedimento" />
									</div>
									<div class="form-group col-md-5">
										<label for="nome">Procedimento</label>
										<input class="form-control"  id="searchProcedimento1" placeholder="Descrição do procedimento" />
									</div>
									
									<div class="form-group col-md-2">
										<label for="nome">Qtd.</label>
										<input class="form-control" type="text" name="quantidade" id="quantidade1"  value="1" onchange="getSaldoDevedor1(this.value)" >
                        			</div>
								</div>
								<div class="row">
									<div class="form-group col-md-2">
										<label for="nome">Ad.%</label>
										<input class="form-control" type="text" name="adicional1" id="adicional1"  value="0" onchange="getSaldoDevedor1(this.value)" >
                        			</div>
									<div class="form-group col-md-2">
										<label for="nome">Red.%</label>
										<input class="form-control" type="text" name="redutor1" id="redutor1"  value="0" onchange="getSaldoDevedor1(this.value)" >
                        			</div>
									
									<div class="form-group col-md-3">
											<label for="nome">Valor R$</label>
											<input class="form-control" name="valor1" id="valor1" value="0">
                        			</div>
									
									<div class="form-group col-md-3">
										<label for="nome">Receb.R$</label>
										<input class="form-control" type="text" name="valorRecebido1" id="valorRecebido1" onchange="getSaldo1(this.value)">
									</div>
									<div class="form-group col-md-2">
										<br>
										<button type="button" id="addRow" class="btn btn btn-sm btn-primary" ><i class="glyphicon glyphicon-plus"></i></button> 
									</div>
								</div>
								
								<div class="row">
										<div class="form-group col-md-6">
											<!--<label for="totalProcedimentos">Total R$</label> <h3 id="total"></h3> -->
								...
								<div class="row">
									...
									<div class="modal-footer">
										...
										<button  type="button" id="btnOperacoesLote" class="btn btn-primary" title="Salvar" onclick="inserirProcedimentos">Salvar</button>
								</form>
							</div>
						</div>
					</div>
								...
								<div class="row">
									<div class="form-group col-md-6">
									  ...
									  <select id="tipoRelatorio" name="tipoRelatorio" class="form-control"> 
											<option value="0">Plano de Saúde</option>
											<option value="1">Eletivas</option>
											<option value="2">Plantões</option>
...
<script>	
    ...
    $('#addRow').on( 'click', function () {
		totalProcedimentos = totalProcedimentos + $("#valor1").maskMoney('unmasked')[0];	
    	totalRecebido = totalRecebido + $("#valorRecebido1").maskMoney('unmasked')[0];	
        
        t.row.add([
		document.getElementById('searchCodProcedimento1').value,
            document.getElementById('searchProcedimento1').value,
			document.getElementById('quantidade1').value,
			document.getElementById('adicional1').value,
			document.getElementById('redutor1').value,
			document.getElementById('valor1').value,
			document.getElementById('valorRecebido1').value,
			'<button type="button" id="btnExcluirItemProcedimento" class="btn  btn-danger btn-xs"><i class="glyphicon glyphicon-trash"></i></button>',
             ]).draw( false );
				counter++;
				$('#totalProcedimentos').html(totalProcedimentos);
				$('#totalRecebido').html(totalRecebido);
			});
	});	
		
	// Simply get the sum of a column
		
	//	alert(t.column(6).data().sum());
	  /* 	 $('#tblprocedimentos').DataTable( {
 drawCallback: function () {
	...
	//document.getElementById('total').innerHTML = document.getElementById('valor').value;
	// Automatically add a first row of data
	// $('#addRow').click();
	//	$("#modalAdicionaProcedimentos").modal();
		...
		$(document).on('click','#btnVisualizar',function(e){
			e.preventDefault();
				   ...
				   var id = document.getElementById("tipoRelatorio").value;
					switch (id) {
						case '0':
							window.open("relatoriosPlanodeSaude.php?id="+idmedico+"&start_date="+start_date+"&end_date="+end_date);
							break;
						
						case '1':
							window.open("relatoriosEletivas.php?id="+id);
							break;
						default:
							text = "No value found";
							}
...
<script>
	function getSaldo(val) {
		...
		if ($('#saldo').maskMoney('unmasked')[0] == 0) {
			var saldo = $('#valor').maskMoney('unmasked')[0];
		...
		saldo = $('#saldo').maskMoney('unmasked')[0] - $('#valorRecebido').maskMoney('unmasked')[0];
		
		saldo = saldo.toFixed(2);
		document.getElementById("saldo").value = saldo;
		$("#saldo").maskMoney('mask');
		if ($('#valorRecebido').maskMoney('unmasked')[0] == 0) {
			document.getElementById("glosa").value = 0;
			$("#glosa").maskMoney('mask');	
		}
			
		else 
		{
			document.getElementById("glosa").value = saldo;
			$("#glosa").maskMoney('mask');	
		}
	}
</script>
...
<script>
	function getSaldoDevedor1(val) {
		//Calcula quantidade 
		// Recebe o valor do procedimento original importado da base de procedimentos/porte
		var valor1= $("#valorProcedimento1").val();
		alert(valor1);
		if (valor != 0) {
			//Calculo para procedimentos cadastrados
			var adicional = $('#adicional1').val();
			var redutor = $('#redutor1').val();
		
			adicional =  ((adicional/100) * ((valor * $('#quantidade1').val())));
			redutor = ((redutor/100) * ((valor * $('#quantidade1').val())));
			valor = ((valor * $('#quantidade1').val())) + adicional - redutor;
			valor = valor.toFixed(2);
			alert (valor);
			
			document.getElementById("valor1").value = valor;
			$("#valor1").maskMoney('mask');
			}
		
		else {
			//Procedimentos lançados manualmente
			valor = $("#valor1").maskMoney('unmasked')[0];
			adicional =  ((adicional/100) * ((valor * $('#quantidade1').val())));
			redutor = ((redutor/100) * ((valor * $('#quantidade1').val())));
			valor = ((valor * $('#quantidade1').val())) + adicional - redutor;
			valor = valor.toFixed(2);
...
// search CÓDIGO PROCEDIMENTO
	 $( function() {
      $( "#searchCodProcedimento1" ).autocomplete({
            source: function( request, response ) {
                ...
                $.ajax({
                    ...
                    dataType: "json",
                    data: {
						searchprocedimento1: request.term, 
						idconvenio : $('#convenio').val()
                        },
                    success: function( data ) {
                        response( data );
			...
			appendTo: "#modalAdicionaProcedimentos",
		
            select: function (event, ui) {
                $('#searchCodProcedimento1').val(ui.item.codigoProcedimento); // display the selected text
				$('#searchProcedimento1').val(ui.item.label);
				$('#idprocedimentos1').val(ui.item.value);	
                $('#codigoProcedimento1').val(ui.item.codigoProcedimento);	
                $('#descricaoProcedimento1').val(ui.item.label);	
				$('#valor1').val( ui.item.value1);
			    $("#valor1").maskMoney('mask');
				$("#quantidade1").val(1);
				$("#valorProcedimento1").val(ui.item.value1);
				
				alert (	$('#valor1').val( ui.item.value1));
			    return false;
            }
        });
...
// search PROCEDIMENTO
	 $( function() {
      $( "#searchProcedimento1" ).autocomplete({
            source: function( request, response ) {
            ...
            select: function (event, ui) {
				$('#searchCodProcedimento1').val(ui.item.codigoProcedimento); // display the selected text
                $('#searchProcedimento1').val(ui.item.label); // display the selected text
				$('#idprocedimentos1').val(ui.item.value);	
                $('#codigoProcedimento1').val(ui.item.codigoProcedimento);	
                $('#descricaoProcedimento1').val(ui.item.label);	
				$('#valor1').val( ui.item.value1);
