				...
				<div class="well">	
						...
						<div class="row"> 
								<div class="form-group col-lg-2">
									...
									<div class="form-group col-lg-1">
										<label for="nome">Ad./Red. %</label>
										<input class="form-control" type="text" name="ajuste" id="ajuste"  value="0" onchange="getSaldoDevedor(this.value)" >
                        			</div>
								...
								<div class="form-group col-lg-1">
									<label for="nome">Glosa/Exced. R$</label>
									<input class="form-control" type="text" style="background-color:pink;" name="glosa" id="glosa">
                        		</div>
								
								<div class="form-group col-lg-1">
									<label for="nome">Saldo R$</label>
									<input class="form-control" type="text" style="background-color:yellow;" name="saldo" id="saldo" value="0" onchange="getSaldo(this.value)">
                        		</div>
								...
								<div class="form-group col-lg-2"> 
										<label class="control-label">Data Cobrança</label>
											<div class='input-group date' id='datetimepicker3'>
											 <input type='text' class="form-control" name="dataCobranca" id="dataCobranca"  value="null"/>
											<span class="input-group-addon">
											<span class="glyphicon glyphicon-calendar"></span>
											</span>
										  </div>
								...
								<div class="form-group col-lg-2"> 
										<label class="control-label">Data Pagamento</label>
											<div class='input-group date' id='datetimepicker4'>
											 <input type='text' class="form-control" name="dataPagamento" id="dataPagamento"  value="null"/>
											<span class="input-group-addon">
											<span class="glyphicon glyphicon-calendar"></span>
											</span>
										  </div>
								...
								<div class="form-group col-lg-2"> 
										<label class="control-label">Data Repasse</label>
											<div class='input-group date' id='datetimepicker6'>
											 <input type='text' class="form-control" name="dataRepasse" id="dataRepasse"  value="null"/>
											<span class="input-group-addon">
											<span class="glyphicon glyphicon-calendar"></span>
											</span>
										  </div>
							...
							<div class="modal-dialog" role="document">
								<div class="modal-content">
									...
									<div class="form-group col-md-6">
									  <label for="tipoRelatorio">Status</label>
									  <select id="tipoRelatorio" name="tipoRelatorio" class="form-control"> 
											<option value="0">FATURAR</option>
									...
									<button type="button" class="btn btn-default" data-dismiss="modal">Fechar</button>
										<button  type="button" id="btnVisualizar" class="btn btn-primary" title="Editar dado" onclick="operacoesLote()">Atualizar </button>
								</form>
							</div>
						</div>
					</div>
							...
							<div class="modal-dialog" role="document">
								<div class="modal-content">
								 ...
								 <form role="form" action="./.php" method='post' enctype="multipart/form-data">
									...
									<div class="modal-footer">
									<button type="button" class="btn btn-default" data-dismiss="modal">Fechar</button>
										<button  type="button" id="btnVisualizar" class="btn btn-primary" title="Editar dado" onclick="relatorios()">Imprimir </button>
...
//https://datatables.net/examples/server_side/select_rows.html
	...
	<script>
		  function relatorios() {
				...
				switch (id) {
						case '0':
							window.open("relatoriosPlanodeSaude.php?id="+id);
							break;
	...
	<script>
			$(document).ready(function(){
			 ...
			 event.preventDefault();
				...
				method:"POST",  
				data: {submit : $("#inserirConta").val(), idempresa : $("#idempresa").val(),  dataRealizacao : $("#dataRealizacao").val(), idpaciente: $("#idpaciente").val(), paciente: $("#searchPaciente").val(), 
					  carteiraPaciente: $("#searchNoCarteira").val(), idmedico: $("#idmedico").val(), medico: $("#searchMedico").val(), convenio : $('#convenio').find('option:selected').text(), hospital : $('#hospital').find('option:selected').text(),
					  codigoProcedimento: $("#searchCodProcedimento").val(), descricaoProcedimento: $("#searchProcedimento").val(), quantidade : $("#quantidade").val(),  ajuste : $("#ajuste").val(),
					  ...
					  formaPagamento: $("#formaPagamento").val(), statusPagamento : $("#inputStatusPagamento").find('option:selected').text()},
				beforeSend:function(){  
					},  
	...
	<script>
		...
		var result = JSON.parse(this.responseText);
				alert( result[0][1]);
				$("#dataRealizacao").val(result[0][1]);
				$("#searchNoCarteira").val(result[0][3]);
				$("#searchPaciente").val(result[0][4]);
				$("#valor").val(result[0][6]);
				$("#valor").maskMoney('mask');
				$("#quantidade").val(result[0][7]);
				$("#ajuste").val(result[0][8]);
				$("#valorRecebido").val(result[0][9]);
		}
	};
	...
	function getSaldo(val) {
		...
		var saldo = $('#valorRecebido').maskMoney('unmasked')[0] - $('#glosa').maskMoney('unmasked')[0] ;
	...
	function getSaldoDevedor(val) {
		...
		if (valor != 0) {
			...
			if (ajuste < 0) {
				// Calcula redução
			ajuste =  ((ajuste/100) * ((valor * $('#quantidade').val())))*-1;
			valor = ((valor * $('#quantidade').val()))-ajuste;
			valor = valor.toFixed(2);
			document.getElementById("valor").value = valor;
			$("#valor").maskMoney('mask');
			}
			else  {
				var ajuste =  (ajuste/100) * ((valor * $('#quantidade').val()));
				valor = ((valor * $('#quantidade').val()))+ajuste;
				valor = valor.toFixed(2);
				document.getElementById("valor").value = valor;
				$("#valor").maskMoney('mask');
			...
			if (ajuste < 0) {
				...
				ajuste =  ((ajuste/100) * ((valor * $('#quantidade').val())));
				valor = ( valor * $('#quantidade').val())+ajuste;
				valor = valor.toFixed(2);
				document.getElementById("valor").value = valor;
				$("#valor").maskMoney('mask');
...
<script>
		$(document).ready(function(){
		 ...
		 {
		  var dataTable = $('#listar-producao').DataTable({
		   "processing" : true,
		   "serverSide" : true,
		   "order" : [],
			   extend: 'collection',
