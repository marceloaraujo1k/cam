...
if(isset($_GET["edit"]))
					{
						$id = $_GET["id"];
						$query = mysqli_query($mysql_conn, "SELECT agendamentos.idconsultas, agendamentos.idpaciente, agendamentos.idempresa, agendamentos.idmedico, agendamentos.dataInicio, agendamentos.idprocedimentos, agendamentos.statusAtendimento,
						pacientes.idpaciente, pacientes.nome, pacientes.sexo, pacientes.idconvenio,
						pacientes.observacao FROM agendamentos INNER JOIN pacientes ON agendamentos.idpaciente = pacientes.idpaciente WHERE agendamentos.idconsultas='$id'");
						$row = mysqli_fetch_assoc($query);
						$form = $row;
						$empresa = getItensTable($mysql_conn,"empresa");
						$convenios = getItensTable($mysql_conn,"convenio");
						...
						<div class="form-group col-md-2">
								...
								<div class="form-group col-md-2">
										<label for="convenio">Médico</label>
											<select id="medicos" name="idmedico" class="form-control">
											<option value=""></option>
											<?php
...
<script>
		$(document).on('click','#btnCaixa',function(e){
            e.preventDefault();
			var x = document.getElementById('medicos').options[document.getElementById('medicos').selectedIndex].text;
